...
14. 支持两种发送模式：
   - 普通问答：输入问题后发送。
15. 左侧品牌区支持一键收起侧边栏，在聊天时可获得更大的可视空间；右上角会出现“展开菜单”按钮用于恢复。
16. 聊天中心左侧面板支持拖拽调整宽度：将鼠标移到面板右侧边缘后拖动，即可改变会话配置区宽度。
17. 会话参数面板支持横向收起：点击“会话”标题旁的小图标可将左侧面板收成窄栏，聊天区会自动铺满；收起时会在顶部栏中间显示会话名+角色的状态条，点击右侧小图标可展开完整参数与列表。
18. 收起后的会话窄栏底部新增 `＋` 按钮，点击即可弹出新建会话窗口。
   - 仅附件模式：问题留空，但上传附件后发送。
