...
5. 右栏多选 MCP，点击 `Save Session Config` 保存当前会话配置。
6. 可上传附件，系统先脱敏再入库。
7. 发送问题时，角色提示词仅作为系统上下文，不会显示在消息流里；回答使用流式输出。
8. 会话支持：`Copy`、`Branch`、`Export MD`、`Export JSON`、`Delete`。
9. 支持批量操作：多选会话后可 `Bulk Export`（ZIP）或 `Bulk Delete`。
10. 支持两种发送模式：
   - 普通问答：输入问题后发送。
   - 仅附件模式：问题留空，但上传附件后发送。
